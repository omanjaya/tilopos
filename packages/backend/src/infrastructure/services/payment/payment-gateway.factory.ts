...
export class PaymentGatewayFactory {
  ...
  getGateway(provider?: string): IPaymentGateway {
    ...
    switch (gateway) {
      case 'midtrans': return this.midtrans;
      case 'xendit': return this.xendit;
      case 'mock':
        if (this.configService.get<string>('NODE_ENV') === 'production') {
          throw new Error('Mock payment gateway cannot be used in production. Set PAYMENT_GATEWAY to midtrans or xendit.');
        }
        return this.mock;
      default:
        throw new Error(`Unknown payment gateway: ${gateway}. Valid options: midtrans, xendit, mock`);
    }
  }
}
