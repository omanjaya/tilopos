...
export class HandleXenditWebhookUseCase {
    ...
    async execute(params: HandleXenditWebhookParams): Promise<HandleXenditWebhookResult> {
        this.logger.log(`Processing Xendit webhook: event=${params.event}, externalId=${params.externalId}`);
        ...
        try {
            ...
            if (payment.status === mappedStatus) {
                this.logger.log(`Payment ${params.externalId} already has status '${mappedStatus}', skipping`);
                return {
                    success: true,
                    event: params.event,
                    externalId: params.externalId,
            ...
            this.logger.error(`Webhook processing failed: ${error instanceof Error ? error.message : error}`);
            throw error;
        }
    }
