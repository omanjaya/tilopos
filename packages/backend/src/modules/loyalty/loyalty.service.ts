...
export class LoyaltyService {
  ...
  async earnPoints(
    ...
    this.logger.log(
      `Customer ${customerId} earned ${pointsEarned} points (${multiplier}x multiplier)`,
    );
  ...
  async checkAndUpgradeTiers(businessId: string): Promise<{
    ...
    for (const customer of customers) {
      ...
      const eligibleTier = await this.repository.getCustomerTier(
        businessId,
  ...
  async processExpiredPoints(businessId: string): Promise<{
    ...
    const expiryThreshold = new Date(
      now.getTime() - Number(program.pointExpiryDays) * 24 * 60 * 60 * 1000,
    );
    ...
    const customerExpiries = new Map<
      string,
      { total: number; currentPoints: number; txIds: string[]; details: typeof result.details }
    >();
