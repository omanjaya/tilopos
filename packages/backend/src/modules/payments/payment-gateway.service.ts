...
export class PaymentGatewayService {
        ...
        try {
            ...
            if (gatewayInfo.type === 'cash') {
                ...
                result = {
                    ...
                    checkoutUrl: gatewayResult.paymentData?.checkoutUrl as string | undefined || gatewayResult.paymentData?.invoiceUrl as string | undefined,
                    expiresAt: gatewayResult.paymentData?.expiresAt ? new Date(gatewayResult.paymentData.expiresAt as string) : undefined,
                };
            }
    ...
    async processWithXendit(
        ...
        const paymentResult: PaymentResult = {
            ...
            checkoutUrl: result.paymentData?.checkoutUrl as string | undefined || result.paymentData?.invoiceUrl as string | undefined,
            expiresAt: result.paymentData?.expiresAt ? new Date(result.paymentData.expiresAt as string) : undefined,
        };

        if (paymentResult.success) {
            await this.createPaymentRecord(transactionId, amount, method as PaymentMethod, paymentResult, 'xendit');
        }
    ...
    async processWithMidtrans(
        ...
        if (paymentResult.success) {
            await this.createPaymentRecord(transactionId, amount, method as PaymentMethod, paymentResult, 'midtrans');
        }
    ...
    async handleCallback(callback: PaymentCallback): Promise<void> {
        ...
        const status = callback.status === 'success' ? 'completed' :
            callback.status === 'failed' ? 'failed' : 'pending';
    ...
    async processRefund(
        ...
        if (gatewayName === 'cash') {
            ...
            const gatewayResult = await gateway.refundPayment(payment.referenceNumber || '', amount, reason);
    ...
    private getGatewayForMethod(method: PaymentMethod): { type: 'cash' | 'edc' | 'gateway'; provider: string } {
        switch (method) {
            case 'cash':
