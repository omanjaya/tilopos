...
export class FeatureService {
    ...
    async toggleFeature(
        ...
        if (!enable) {
            const dependentFeatures = FEATURE_REGISTRY.filter(
                (f) => f.dependencies?.includes(featureKey),
            );
     ...
     */
    async initializeFeaturesForBusinessType(
        businessId: string,
        businessType: string,
    ): Promise<void> {
     ...
     */
    async getFeaturesByCategory(businessId: string): Promise<Record<FeatureCategory, BusinessFeatureDto[]>> {
